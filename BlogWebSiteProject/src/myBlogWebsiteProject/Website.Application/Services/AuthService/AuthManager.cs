...
namespace Website.Application.Services.AuthService
{
    public class AuthManager : IAuthService
    {
        ...
        public async Task<AccessToken> CreateAccessToken(User user)
        {
            IList<OperationClaim> operationClaims = await _userOperationClaimRepository.Query().AsNoTracking()
                .Where(p => p.UserId == user.Id).Select(p =>
                    new OperationClaim
                    {
        ...
        private async Task SendAuthenticatorCodeWithEmail(User user)
        {
            ...
            if (!emailAuthenticator.IsVerified) throw new BusinessException("Email Authenticator must be verified.");
